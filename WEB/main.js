document.addEventListener("DOMContentLoaded", () => {
    ...
    function addPopUpItems(popUpItems, food) {
        ...
        popUpItems.map(item => {
            popUpBox.innerHTML += `<div class="pop-up_item">
                                        ...
                                        <div class="pop-up_dish-discribe">
                                            ...
                                            <div class="pop-up_order-box"
                                            ><button class="pop-up_btn">Замовити</button><span
                                                class="pop-up_price"
        ...
        const popUpButtons = document.querySelectorAll('.pop-up_btn');

        [].forEach.call(popUpButtons, function(el){
            el.addEventListener('click', function(event) {
                const item = this.closest('.pop-up_item');

                const itemToSave = {
                    name: item.children[0].children[0].innerHTML,
                    imgSrc: item.children[0].children[1].getAttribute('src'),
                    price: item.children[2].children[1].innerHTML.split(" ")[0],
                }
                
                storageFood = []

                if(localStorage.getItem('basket')){
                    storageFood = JSON.parse(localStorage.getItem('basket'))
                    console.log(storageFood);
                }

                storageFood.push(itemToSave);

                localStorage.setItem('basket', JSON.stringify(storageFood))
